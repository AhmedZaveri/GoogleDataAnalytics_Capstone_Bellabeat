...
getwd()
setwd("~/Zaveri Data/TUM/9th Semester/Google Data Analytics/Capstone - Bellabeat/Datasets")
...
heartrate$Time_new <- format(strptime(heartrate$datetime[,2], "%I:%M:%S %p"), format="%H:%M:%S")
heartrate <- heartrate[-4]
heartrate$Time_new <- as_hms(heartrate$Time_new)

hourly_calories$datetime <- str_split_fixed(hourly_calories$ActivityHour, " ", n = 2)
hourly_calories$Date <- as.Date(hourly_calories$ActivityHour, "%m/%d/%Y")
hourly_calories$time_new <- format(strptime(hourly_calories$datetime[,2], "%I:%M:%S %p"), format="%H:%M:%S")
...
hourly_steps$Time <- format(strptime(hourly_steps$datetime[,2], "%I:%M:%S %p"), format="%H:%M:%S")
...
ggplot(data = daily_activity, aes(x = factor(Weekday, days_of_the_week), y = TotalSteps)) + 
       geom_bar(stat = "summary", fun = "mean") + labs(title = "Average Daily Steps on Each Day of The Week") + 
       xlab("Day of the Week") + ylab("Average No. of Steps") 

#Changing ID Column to double to character
...
#Adding new column to distribute different 
...
#Average Steps of Each Individual
ggplot(data = AverageSteps) + geom_col(mapping = aes(x = Id, y = AvgSteps, fill = ActivityIntensity)) + 
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
       labs(title = "Average Steps of Each Individual") + 
       xlab("ID of Individual") + ylab("Average No. of Steps") +
       geom_hline(yintercept = mean(AverageSteps$AvgSteps)) + scale_fill_discrete(name = "Intensity of Actiivty")
#OR
...
daily_activity_long <- gather(daily_activity, key = "Activitytype", value = "Minutes",11:14)

#Minutes Spent on Each Type of Activity by Each Individual
ggplot(data = daily_activity_long, aes(fill = Activitytype, x = Id, y = Minutes)) + 
       geom_bar(position = "stack", stat = "summary", fun = "mean") + 
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
       labs(title = "Minutes Spent on Each Type of Activity by Each Individual") + 
       xlab("ID of Individual") + ylab("Minutes of Activity") + 
...
AverageActivityMinutes <- daily_activity_long %>% select(Activitytype, Minutes) %>% 
  group_by(Activitytype) %>% summarise(mean(Minutes))
AverageActivityMinutes <- AverageActivityMinutes %>% rename("AvgMins" = "mean(Minutes)")
...
AverageHourlySteps <- hourly_steps %>% select(Time, StepTotal) %>% 
  ...
  AverageSteps <- daily_activity %>% select(Id, TotalSteps) %>% group_by(Id) %>% 
  summarise(mean(TotalSteps))
AverageHourlySteps <- AverageHourlySteps %>% rename("AvgSteps" = "mean(StepTotal)")
...
ggplot(data = AverageHourlySteps) + geom_col(aes(x = Time, y = AvgSteps, fill = StepsIntensity)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  labs(title = "Average Steps by Hour") + xlab("Time of the Day") + ylab("Average Steps") + 
  scale_fill_discrete(name = "Hours")

#Average Minutes of Sleep on Each Day of the Week
ggplot(data = sleep, aes(x = factor(Weekday, days_of_the_week), y = TotalMinutesAsleep)) + 
  geom_bar(stat = "summary", fun = "mean") + labs(title = "Average Minutes of Sleep on Each Day of The Week") + 
  xlab("Day of the Week") + ylab("Average Minutes of Sleep") 

#Average Sedentary minutes on Each Day of the Week
ggplot(data = daily_activity, aes(x = factor(Weekday, days_of_the_week), y = SedentaryMinutes)) + 
  geom_bar(stat = "summary", fun = "mean") + labs(title = "Average Sedentary Minutes on Each Day of The Week") + 
...
SedentaryMinutes <- SedentaryMinutes %>% rename("AvgSedentaryMins" = "mean(SedentaryMinutes)")
SedentaryMinutes$AvgSedentaryMins <- round(SedentaryMinutes$AvgSedentaryMins, 0)

SleepingMinutes <- sleep %>% select(Weekday, TotalMinutesAsleep) %>% group_by(Weekday) %>% 
  summarise(mean(TotalMinutesAsleep))
SleepingMinutes <- SleepingMinutes %>% rename("AvgSleepingMins" = "mean(TotalMinutesAsleep)")
SleepingMinutes$AvgSleepingMins <- round(SleepingMinutes$AvgSleepingMins, 0)

merge_sleep_sedentary <- merge(SleepingMinutes, SedentaryMinutes, by = "Weekday")
merge_sleep_sedentary <- merge_sleep_sedentary %>% mutate(Total_Mins = 1440)
merge_sleep_sedentary_long <- gather(merge_sleep_sedentary, key = "SleepSed", value = "Minutes", 2:4)
merge_sleep_sedentary_long <- merge_sleep_sedentary_long %>% mutate(Percentage = Minutes * 100 / 1440)
merge_sleep_sedentary_long$Percentage <- round(merge_sleep_sedentary_long$Percentage, 1) 

ggplot(data = merge_sleep_sedentary_long, aes(x = factor(Weekday, days_of_the_week),
       y = Minutes, fill = SleepSed)) + geom_bar(position="dodge", stat="identity") + 
       geom_text(aes(label = Minutes), position = position_dodge(0.9)) + xlab("Day of the Week") + 
       scale_fill_discrete(name = "Legend", breaks = c("Total_Mins", 
       "AvgSedentaryMins", "AvgSleepingMins"), labels = c("Total Minutes", 
       "Avg. Sedentary Mins", "Avg. Sleeping Mins")) + 
       labs(title = "Sleeping Minutes vs. Sedentary Minutes Relative to Total Minutes") + 
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
